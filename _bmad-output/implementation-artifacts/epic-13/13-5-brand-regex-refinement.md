...
- [ ] Acceptance test scenarios created with triple tags (@E-013-S-N, @FR-SCORE-27, @story-13-5)
- [ ] Feature file: `test/acceptance/features/E-013-scoring-algorithm-improvements.feature`
- [ ] Step definitions: `test/acceptance/step_definitions/E-013-brand-regex.steps.ts`
...
- [ ] Task 1: Refactor brand detection to title-only
  - [ ] 1.1 Refactor internal matching: stop using `fullText` (which combines title+description at line 177) for `VALUE_KEYWORDS` matching. Use `title.toLowerCase()` instead. No function signature change needed — `estimateValue()` already accepts separate `title` and `description` parameters.
  - [ ] 1.2 Apply `VALUE_KEYWORDS` patterns to title only
  - [ ] 1.3 Keep `RISK_KEYWORDS` applied to `title + ' ' + description`

- [ ] Task 2: Add negative pattern support
  - [ ] 2.1 Extend `VALUE_KEYWORDS` type: `{ pattern: RegExp, boost: number, label: string, tag: string, negativePatterns?: RegExp[] }`
  - [ ] 2.2 Add negatives for each brand:
  ...
  - [ ] 2.3 In matching logic: if pattern matches AND any negative pattern also matches → skip the boost

- [ ] Task 3: Sealed/NIB contextual matching
  - [ ] 3.1 For "sealed" and "new in box" patterns: match title OR first 100 chars of description
  - [ ] 3.2 Add helper: `matchTitleOrLeadDescription(text, title, description, leadChars = 100)`

- [ ] Task 4: Unit tests
  - [ ] 4.1 Test: "Nintendo Switch OLED 64GB" → nintendo boost applied (true brand in title)
  - [ ] 4.2 Test: "Phone case compatible with Nintendo Switch" → NO boost (negative pattern)
  - [ ] 4.3 Test: "Vintage Pyrex Mixing Bowl" → vintage boost applied (true vintage in title)
  - [ ] 4.4 Test: "Modern lamp, vintage-style design" → NO boost (negative pattern)
  - [ ] 4.5 Test: "Sealed iPhone 15" → sealed boost (in title)
  - [ ] 4.6 Test: "Used iPhone 15... original box sealed" at char 200 → NO sealed boost
  - [ ] 4.7 Test: "Broken PS5 for parts" → risk penalty applied (from description scan)
  - [ ] 4.8 Test: "PS5 Console" with description "one scratch on top, works great" → cosmetic-wear risk applied (from description)
