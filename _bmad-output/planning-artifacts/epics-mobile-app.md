...
FR-MOBILE-FOUNDATION-03: Mobile workspace shall use TypeScript strict mode with `tsconfig.json` extending the project base config, and shall reuse the path alias convention (`@/*` → `mobile/src/*`, `@shared/*` → `packages/*/src` — single wildcard, barrel imports via each package's `src/index.ts`; TypeScript rejects two `*` per `paths` substitution with TS5062, see ADR-15.1-C in Story 15.1)
