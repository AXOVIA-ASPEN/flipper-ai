...
Given(/^a set of eBay sold prices \[(.+)\]$/, function (priceList: string) {
...
Then('the price {int} should be excluded from filtered results', function (price: number) {
...
Then('the outliers removed count should be {int}', function (expected: number) {
  assert.strictEqual(
...
Then('the low sample size flag should be true', function () {
  assert.strictEqual(this.filterResult.lowSampleSize, true, 'Expected lowSampleSize to be true');
});

Then('the low sample size flag should be false', function () {
  assert.strictEqual(this.filterResult.lowSampleSize, false, 'Expected lowSampleSize to be false');
});

Then('all {int} prices should be retained', function (count: number) {
  assert.strictEqual(
...
Then('downstream confidence should be set to {string}', function (expectedConfidence: string) {
