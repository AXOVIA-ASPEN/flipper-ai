...
When(
  'I click the {string} button in the Profile section',
...
Then(
  'my display name should be updated to {string}',
...
Then(
  'the API key should display as masked showing only the last 4 characters',
...
When(
  'I set the discount threshold to {int}',
...
When(
  'I send a PATCH to {string} with name {string}',
  async function (this: CustomWorld, endpoint: string, name: string) {
...
Given(
  'an authenticated user navigates to {string}',
...
Then(
  'the settings page displays Profile, Notifications, AI Preferences, and API Keys sections',
  async function (this: CustomWorld) {
...
When(
  'the user updates their display name to {string}',
  async function (this: CustomWorld, name: string) {
    const input = this.page.locator('#profile-name');
    await input.fill(name);
  }
...
Then(
  'the display name is updated and a success message appears',
  async function (this: CustomWorld) {
    const success = this.page.locator('text=saved successfully');
    await expect(success).toBeVisible({ timeout: 5000 });
  }
...
When(
  'the user selects {string} as AI model and sets discount threshold to {int}',
  async function (this: CustomWorld, model: string, _threshold: number) {
    const select = this.page.locator('#llm-model');
    const value = model.toLowerCase().replace(/ /g, '-').replace('gpt-', 'gpt-');
    await select.selectOption(value);
  }
...
When(
  'the user enters an OpenAI API key and clicks {string}',
  async function (this: CustomWorld, _buttonText: string) {
    const input = this.page.locator('#api-key-input');
...
Then(
  'the masked key is displayed showing only the last 4 characters',
  async function (this: CustomWorld) {
    const masked = this.page.locator('text=/••••/');
    await expect(masked).toBeVisible({ timeout: 5000 });
  }
...
Then(
  'the API key is removed and {string} is shown',
  async function (this: CustomWorld, text: string) {
