...
Given('the llm-analyzer source file at {string}', function (filePath: string) {
...
When('I inspect the analyzeSellability OpenAI call configuration', function () {
...
Then('it should include response_format with type {string}', function (formatType: string) {
...
Then('the system prompt should contain the word {string}', function (keyword: string) {
...
When('I search for regex-based JSON extraction patterns', function () {
  ...
  this.hasDirectParse =
    this.sourceContent.includes('JSON.parse(responseText)') ||
    this.sourceContent.includes('JSON.parse(retryText)');
});

// ==================== Then: AC #2 — Regex extraction removed ====================

Then('no regex extraction pattern should be found', function () {
  assert.strictEqual(
...
Then('direct JSON.parse should be used for response parsing', function () {
  assert.strictEqual(
    this.hasDirectParse,
    true,
...
When('I inspect the JSON parse error handling in analyzeSellability', function () {
  const fnStart = this.sourceContent.indexOf('export async function analyzeSellability');
  assert(fnStart > -1, 'analyzeSellability function not found');
  this.fnBody = this.sourceContent.substring(fnStart);
});
...
Then('there should be a retry with a simplified prompt on parse failure', function () {
...
Then('on second failure it should return null for algorithmic fallback', function () {
...
When('I inspect the error logging in the retry catch block', function () {
  const fnStart = this.sourceContent.indexOf('export async function analyzeSellability');
  assert(fnStart > -1, 'analyzeSellability function not found');
  this.fnBody = this.sourceContent.substring(fnStart);
});
...
Then('Sentry.captureException should be called with the error', function () {
  assert(
...
Then('the extra context should include the original response text', function () {
...
When('I inspect the SellabilityAnalysis interface and buildResult function', function () {
...
Then('all original fields should be preserved in the response schema', function () {
...
Then('the buildResult validation functions should still be applied', function () {
  assert(this.hasBuildResult, 'buildResult function not found');

  const requiredValidators = ['validateDemandLevel', 'validateRisk', 'validateConfidence'];
