...
Then('the source declares the canonical .fp-glass design system surface', function () {
...
Then('both routes implement HTTP method handlers for the configured-filter contract', function () {
...
Then('the routes integrate Zod validation via {string}', function (schemaName: string) {
  const collection = this.searchConfigsRoute as string;
...
Then('the source emits Largest Contentful Paint events for budget enforcement', function () {
...
Then('every declared scraper timeout is at most {int}_000 ms', function (maxSeconds: number) {
  ...
  for (const { platform, source } of configs) {
    ...
    for (const m of matches) {
      ...
      expect(
        value,
        `${platform} declared ${value}ms timeout — exceeds ${max}ms`
      ).toBeLessThanOrEqual(max + 1);
    }
  }
});
...
Then('the emitter writes events synchronously to the connected response stream', function () {
...
Given(/^each scraper's scraper\.ts in "([^"]+)"$/, function (scrapersRoot: string) {
  const root = path.join(PROJECT_ROOT, scrapersRoot);
  const platforms = fs
    .readdirSync(root, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name);
  this.scraperRetryConfigs = platforms.flatMap((platform) => {
    const candidates = [
...
Then('every scraper config declares retries and a non-zero retry delay', function () {
  const configs = this.scraperRetryConfigs as Array<{
    platform: string;
    file: string;
    source: string;
  }>;
...
Then('both routes export a GET handler that responds with the service status', function () {
...
Then('the logger is built on the pino library and exports a singleton', function () {
...
Then('the workflow sets Cloud Run --max-instances and --min-instances flags', function () {
...
Then('the cache TTL is configured to {int} hours', function (hours: number) {
...
Then('the config sets a Strict-Transport-Security header on responses', function () {
...
Given('the auth library that hashes passwords', function () {
  ...
  if (
    this.bcryptCallSites.length === 0 ||
    !this.bcryptCallSites.some((s) => /bcrypt(?:js)?/.test(s))
  ) {
...
Then('bcryptjs.hash is invoked with cost factor {int}', function (cost: number) {
  const sources = this.bcryptCallSites as string[];
  const found = sources.some((src) =>
    new RegExp(`bcrypt(?:js)?\\.hash\\([^)]*,\\s*${cost}\\b`).test(src)
  );
...
Then('the rate limiter exports a per-IP token bucket helper', function () {
  ...
  expect(src).toMatch(
    /export\s+(?:async\s+)?(?:function|const)\s+(?:rateLimit|checkRateLimit|withRateLimit|consume)/
  );
});
...
Then('the cookie is set with httpOnly true and sameSite Strict', function () {
...
Then('the handler calls stripe.webhooks.constructEvent with the request signature', function () {
...
Then('the tracker exports a hCaptcha verification helper', function () {
...
Then('the projects array includes chromium, firefox, and webkit engines', function () {
...
Then('the matrix references every epic from FR-INFRA through FR-UI-DESIGN', function () {
...
Then('the stylesheet imports tailwindcss and ships the canonical fp-* design tokens', function () {
...
Then('the layout renders a <main> landmark and a skip-to-main-content link', function () {
