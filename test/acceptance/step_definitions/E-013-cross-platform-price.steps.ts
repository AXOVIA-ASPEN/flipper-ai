...
Given('eBay sold prices of [{int}] for {string}', function (p1: number, query: string) {
  if (!this.state) this.state = {} as ScenarioState;
  this.state.searchQuery = query;
...
Given('Facebook active prices of [{int}] for {string}', function (p1: number, _query: string) {
  if (!this.state) this.state = {} as ScenarioState;
...
Given('raw prices of [{int}] on each platform', function (price: number) {
  if (!this.state) this.state = {} as ScenarioState;
...
Given('Mercari fetcher throws an error', function () {
  if (!this.state) this.state = {} as ScenarioState;
...
When('cross-platform price intelligence is fetched for {string}', async function (query: string) {
  ...
  const ebayData =
    state.ebayPrices?.length > 0 ? buildPlatformData('ebay', 'sold', state.ebayPrices, 100) : null;

  // Fetch non-eBay platforms
  const otherResult = await fetchCrossPlatformPrice(query, undefined, fetchers, {
    skipCache: true,
  });
...
When('the price intelligence override is applied', function () {
  const state = this.state as ScenarioState;
...
When('the rescue check is applied with threshold {int}', function (threshold: number) {
  const state = this.state as ScenarioState;
  const mockResult: CrossPlatformPriceResult = {
    verifiedMarketValue: state.verifiedMarketValue,
    confidence: 'high',
...
When('platform fee normalization is applied', function () {
  const state = this.state as ScenarioState;
...
Then('total sold comps should be at least {int}', function (minComps: number) {
  const state = this.state as ScenarioState;
  assert.ok(state.result, 'Expected non-null result');
  assert.ok(
...
Then(
  ...
  function (low: number, high: number) {
    ...
    assert.ok(state.result, 'Expected non-null result');
    const vmv = state.result.verifiedMarketValue;
    assert.ok(vmv >= low && vmv <= high, `Expected VMV between ${low} and ${high}, got ${vmv}`);
  }
);

Then('the confidence level should be {string}', function (expected: string) {
  const state = this.state as ScenarioState;
  assert.ok(state.result, 'Expected non-null result');
  assert.strictEqual(state.result.confidence, expected);
});

Then('the overridden score should be greater than {int}', function (threshold: number) {
  const state = this.state as ScenarioState;
...
Then('eBay net price should be {int}', function (expected: number) {
  const state = this.state as ScenarioState;
...
Then('Mercari net price should be {int}', function (expected: number) {
  const state = this.state as ScenarioState;
...
Then('Facebook net price should be {int}', function (expected: number) {
  const state = this.state as ScenarioState;
...
Then('OfferUp net price should be {int}', function (expected: number) {
  const state = this.state as ScenarioState;
...
Then('Craigslist net price should be {int}', function (expected: number) {
  const state = this.state as ScenarioState;
...
Then('the result should use cached data', function () {
  const state = this.state as ScenarioState;
...
Then('no platform fetchers should be invoked', function () {
  const state = this.state as ScenarioState;
  assert.strictEqual(state.fetchMarketPriceCalled, false, 'Expected no fetcher calls (cache hit)');
});

Then('the result should still contain eBay data', function () {
  const state = this.state as ScenarioState;
  assert.ok(state.result, 'Expected non-null result');
  const ebay = state.result.platformData.find((pd) => pd.platform === 'ebay');
...
Then('the result should not be null', function () {
  const state = this.state as ScenarioState;
  assert.ok(state.result, 'Expected non-null result');
});
