...
Then('it should pass {string} as a launch argument', function (arg: string) {
...
Then('all user agents should reference Chrome version 130 or higher', function () {
  const typesContent = readFile('src/scrapers/craigslist/types.ts');
...
Then('it should include a {string} field of type number', function (field: string) {
...
Then('it should rotate user agents from a pool of current versions', function () {
  expect(this.fileContent).toContain('getRandomUserAgent');
...
Then('it should set navigator.webdriver to false via addInitScript', function () {
  expect(this.fileContent).toContain('addInitScript');
  expect(this.fileContent).toContain('webdriver');
});

Then('it should use randomized viewport dimensions between 1280x800 and 1920x1080', function () {
...
Given('the scraper module directory at {string}', function (dirPath: string) {
...
Then('{string} should exist as the public entry point', function (filePath: string) {
  expect(fileExists(filePath)).toBe(true);
  const content = readFile(filePath);
  expect(content).toContain('export');
});

Then('{string} should contain the core scraping logic', function (filePath: string) {
  expect(fileExists(filePath)).toBe(true);
  const content = readFile(filePath);
  expect(content).toMatch(/export async function scrape\w+/);
});

Then('{string} should contain TypeScript interfaces', function (filePath: string) {
  expect(fileExists(filePath)).toBe(true);
  const content = readFile(filePath);
...
Given('the scraper module exports {string}', function (exportName: string) {
...
Then('the route should check for existing RUNNING jobs before starting a new one', function () {
  expect(this.routeContent).toContain('hasRunningJob');
});

Then('it should return a 403 error if a job is already running', function () {
  expect(this.routeContent).toContain('ForbiddenError');
});

// ==================== Zero-results detection ====================

Given('the scraper response type includes {string}', function (field: string) {
  const content = readFile('src/scrapers/craigslist/types.ts');
  expect(content).toContain(field);
});

When('the page loads but no listings are extracted', function () {
...
Then('the failure reason should be {string}', function (reason: string) {
...
Then('it should use {string} for response formatting', function (funcName: string) {
  expect(this.routeContent).toContain(funcName);
});
