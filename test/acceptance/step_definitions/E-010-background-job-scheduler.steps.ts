...
import { buildDeduplicationKey } from '../../../src/lib/notification-events';
import {
...
Then(
  'the monitoring route source enforces a minimum key length of {int}',
...
Given(
  'a listing id {string} and event type {string}',
  function (listingId: string, eventType: string) {
...
Then('the source contains per-listing error handling that does not abort the batch', function () {
  assert.ok(state.routePath, 'routePath not set');
  const src = fs.readFileSync(state.routePath, 'utf-8');
  assert.match(
    src,
...
Then('the source catches P2002 and throws an error with code {string}', function (code: string) {
  assert.ok(state.routePath, 'routePath not set');
  const src = fs.readFileSync(state.routePath, 'utf-8');
  assert.match(
    src,
...
Then('the source does not use findFirst before create for the concurrency guard', function () {
  assert.ok(state.routePath, 'routePath not set');
  const src = fs.readFileSync(state.routePath, 'utf-8');
  assert.match(
    src,
...
Then('the source transitions stale RUNNING jobs to FAILED before starting a new run', function () {
  assert.ok(state.routePath, 'routePath not set');
  const src = fs.readFileSync(state.routePath, 'utf-8');
  assert.match(
    src,
...
Given(
  'a platform with {int} checks and {int} marked unavailable',
  function (checked: number, unavailable: number) {
    state.platformStats = {};
...
When(
  'isAnomalyThresholdExceeded is evaluated at a {int} percent threshold',
