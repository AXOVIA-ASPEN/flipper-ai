...
test.describe('Account Management - BDD Tests', () => {
  ...
  test.describe('Feature: View Account Profile', () => {
    test('Given I am logged in, When I navigate to account settings, Then I should see my profile information', async ({
      page,
    }) => {
    ...
    test('Given I am on the account page, When I view my profile, Then I should see my account creation date', async ({
      page,
    }) => {
  ...
  test.describe('Feature: Update Profile Name', () => {
    test('Given I am logged in, When I update my name to "Updated Name", Then my profile should reflect the new name', async ({
      page,
    }) => {
      // Given I am logged in
      await page.goto('/settings');
    ...
    test('Given I try to update my name, When I provide an empty name, Then I should see a validation error', async ({
      page,
    }) => {
      ...
      if ((await nameInput.count()) > 0) {
        ...
        if ((await saveButton.count()) > 0) {
          ...
          await expect(
            page.getByText(/name.*required/i).or(page.getByText(/please.*name/i))
  ...
  test.describe('Feature: Change Email Address', () => {
    test('Given I want to change my email, When I update to a new email, Then I should receive a verification request', async ({
      page,
    }) => {
      ...
      if ((await emailInput.count()) > 0) {
        ...
        if ((await saveButton.count()) > 0) {
          ...
          await expect(
            page.getByText(/verification.*sent/i).or(page.getByText(/check.*email/i))
          ).toBeVisible({ timeout: 3000 });
        }
      }
    });

    test('Given I try to change email, When I provide an invalid email format, Then I should see an error', async ({
      page,
    }) => {
      ...
      if ((await emailInput.count()) > 0) {
        ...
        if ((await saveButton.count()) > 0) {
          ...
          await expect(
            page.getByText(/invalid.*email/i).or(page.getByText(/email.*valid/i))
          ).toBeVisible({ timeout: 2000 });
        }
      }
    });
  ...
  test.describe('Feature: Change Password', () => {
    test('Given I want to change my password, When I navigate to security settings, Then I should see password change option', async ({
      page,
    }) => {
      ...
      const passwordSection = page.getByText(/password/i).or(page.getByText(/security/i));

      // Then I should see password change option
      await expect(passwordSection.first()).toBeVisible();
    });

    test('Given I am changing my password, When I provide current and new passwords, Then my password should be updated', async ({
      page,
    }) => {
      ...
      if ((await currentPasswordInput.count()) > 0) {
        ...
        const changePasswordButton = page
          ...
          .or(page.locator('button:has-text("Update Password")'));
        ...
        if ((await changePasswordButton.count()) > 0) {
          ...
          await expect(
            page.getByText(/password.*updated/i).or(page.getByText(/success/i))
          ).toBeVisible({ timeout: 3000 });
        }
      }
    });

    test('Given I am changing my password, When I provide incorrect current password, Then I should see an error', async ({
      page,
    }) => {
      // Given I am changing my password
      await page.goto('/settings');
      ...
      if ((await currentPasswordInput.count()) > 0) {
        ...
        if ((await changePasswordButton.count()) > 0) {
          ...
          // Then I should see an error
          await expect(
            page.getByText(/current.*password.*incorrect/i).or(page.getByText(/password.*wrong/i))
          ).toBeVisible({ timeout: 3000 });
        }
      }
    });

    test('Given I am setting a new password, When the password is too weak, Then I should see validation requirements', async ({
      page,
    }) => {
      ...
      if ((await newPasswordInput.count()) > 0) {
        ...
        await expect(
          page
            .getByText(/password.*8.*characters/i)
            .or(page.getByText(/password.*strong/i))
            .or(page.getByText(/password.*must/i))
        ).toBeVisible({ timeout: 2000 });
      }
    });
  });
  ...
  test.describe('Feature: Delete Account', () => {
    test('Given I want to delete my account, When I access account settings, Then I should see a delete account option', async ({
      page,
    }) => {
      ...
      const deleteOption = page
        ...
        .or(page.getByText(/close.*account/i))
    ...
    test('Given I initiate account deletion, When I confirm deletion, Then I should see a confirmation dialog', async ({
      page,
    }) => {
    ...
    test('Given I am in the delete confirmation dialog, When I cancel, Then my account should remain active', async ({
      page,
    }) => {
      ...
      if ((await deleteButton.count()) > 0) {
        ...
        if ((await cancelButton.count()) > 0) {
          ...
          await expect(page.getByText(/confirm.*delete/i)).not.toBeVisible({ timeout: 2000 });
    ...
    test('Given I confirm account deletion, When the deletion is processed, Then I should be logged out and redirected', async ({
      page,
    }) => {
      ...
      if ((await deleteButton.count()) > 0) {
        ...
        if ((await confirmButton.count()) > 0) {
          ...
          await expect(
            page.getByText(/account.*deleted/i).or(page.getByText(/goodbye/i))
  ...
  test.describe('Feature: Account Security Indicators', () => {
    test('Given I view my account, When I check security status, Then I should see email verification status', async ({
      page,
    }) => {
      ...
      await expect(page.getByText(/verified/i).or(page.getByText(/email.*status/i))).toBeVisible({
        timeout: 3000,
      });
    });

    test('Given I am reviewing security, When I check last login, Then I should see account activity information', async ({
      page,
    }) => {
  ...
  test.describe('Feature: Profile Form Validation', () => {
    test('Given I am updating my profile, When I submit without changes, Then no API call should be made', async ({
      page,
    }) => {
    ...
    test('Given I am editing my profile, When I navigate away, Then I should see an unsaved changes warning', async ({
      page,
    }) => {
  ...
  test.describe('Feature: Accessibility - Account Management', () => {
    test('Given I navigate account settings with keyboard, When I use Tab key, Then all interactive elements should be accessible', async ({
      page,
    }) => {
    ...
    test('Given account management forms, When I use screen reader, Then labels should be properly associated', async ({
      page,
    }) => {
