...
test.describe('Feature: Loading States & Transition Indicators', () => {
  test.describe('Scenario: Dashboard shows loading state while fetching listings', () => {
    test('Given I navigate to the dashboard, When the API is slow, Then I see a loading indicator before content appears', async ({
      page,
    }) => {
      ...
      const loadingIndicator = page.locator(
        '[data-testid="loading"], .animate-spin, text=Loading, [role="progressbar"]'
      );
  ...
  test.describe('Scenario: Opportunities page shows loading state', () => {
    test('Given I navigate to opportunities, When the API responds slowly, Then I see a loading indicator', async ({
      page,
    }) => {
  ...
  test.describe('Scenario: Scraper page shows loading during job submission', () => {
    test('Given I am on the scraper page, When I submit a scrape job, Then the submit button shows a loading state', async ({
      page,
    }) => {
      ...
      const scrapeButton = page.locator(
        'button:has-text("Scrape"), button:has-text("Start"), button:has-text("Run")'
      );
      if (
        await scrapeButton
          .first()
        ...
        const disabledOrSpinning = page.locator(
          'button:disabled .animate-spin, button[disabled]:has-text("Scrape"), button[disabled]:has-text("Start")'
        );
        // Verify loading state appears (or button remains interactive — both valid UX)
        const hasLoadingState = await disabledOrSpinning
          .first()
  ...
  test.describe('Scenario: Analytics page handles slow data gracefully', () => {
    test('Given I navigate to analytics, When data takes time to load, Then the page does not show an error', async ({
      page,
    }) => {
  ...
  test.describe('Scenario: Dashboard refresh button shows loading state', () => {
    test('Given I am on the dashboard with listings, When I click refresh, Then the refresh icon spins', async ({
      page,
    }) => {
      let callCount = 0;
      await page.route('**/api/listings**', async (route) => {
      ...
      const refreshButton = page.locator(
        'button:has-text("Refresh"), button[aria-label="Refresh"], button:has(.lucide-refresh-cw)'
      );
      if (
        await refreshButton
          .first()
        ...
        const hasSpin = await spinningIcon
          .first()
          .isVisible({ timeout: 2000 })
          .catch(() => false);
  ...
  test.describe('Scenario: Bulk action buttons show loading during operations', () => {
    test('Given I have selected listings, When I perform a bulk action, Then the action button shows a loading indicator', async ({
      page,
    }) => {
      ...
      await page.route('**/api/opportunities**', async (route) => {
        if (route.request().method() === 'POST') {
          ...
          return route.fulfill({
            status: 201,
            contentType: 'application/json',
            body: JSON.stringify({ id: 'opp-new' }),
          });
        }
        return route.fulfill({
          status: 200,
      ...
      if (await checkbox.isVisible().catch(() => false)) {
        ...
        if (
          ...
          const bulkButton = page.locator(
            'button:has-text("Opportunities"), button:has-text("Add")'
          );
          if (
            await bulkButton
              .first()
