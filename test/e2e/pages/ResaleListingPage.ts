...
export class ResaleListingPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.descriptionInput = page.locator(
      '[name="description"], [data-testid="listing-description"]'
    );
    ...
    this.submitButton = page.locator(
      'button[type="submit"], button:has-text("Post"), button:has-text("Create")'
    );
    this.cloneButton = page
      .locator('button, a')
      .filter({ hasText: /clone|copy|duplicate/i })
      .first();
    this.priceSuggestion = page.locator(
      '[data-testid="price-suggestion"], .price-suggestion, text=/suggested|recommended/i'
    );
    this.platformSelect = page
  ...
  async getFirstListingLink(): Promise<Locator> {
    return this.page
      .locator('a, tr, [data-testid="listing-row"]')
      .filter({ hasText: /\$/ })
      .first();
  }
}
