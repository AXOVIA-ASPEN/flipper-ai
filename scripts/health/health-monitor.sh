...
#
# Add to crontab:
#   */5 * * * * /path/to/flipper-ai/scripts/health/health-monitor.sh >> /var/log/flipper-health.log 2>&1
#
# Usage:
...
echo -n "[$TIMESTAMP] Checking $READY_URL ... "

START_R=$(date +%s%N)
READY_RESPONSE=$(curl -sf --max-time "$MAX_RESPONSE_TIME" "$READY_URL" 2>&1)
READY_EXIT=$?
END_R=$(date +%s%N)

READY_MS=$(( (END_R - START_R) / 1000000 ))

if [ $READY_EXIT -ne 0 ]; then
  echo "❌ READINESS FAILED (curl exit $READY_EXIT, ${READY_MS}ms)"
  exit 1
fi

READY_STATUS=$(echo "$READY_RESPONSE" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('status','unknown'))" 2>/dev/null)

if [ "$READY_STATUS" = "ready" ]; then
  echo "✅ READY (${READY_MS}ms)"
else
  echo "⚠️  NOT READY: status=$READY_STATUS (${READY_MS}ms)"
  exit 2
fi
