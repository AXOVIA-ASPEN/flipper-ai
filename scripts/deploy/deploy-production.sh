...
run_tests() {
    ...
    # Run linter
    log "Running ESLint..."
    pnpm lint || error "ESLint failed - fix errors before deploying"
    success "ESLint passed"

    # Run type check
    log "Running TypeScript type check..."
    pnpm exec tsc --noEmit || error "TypeScript compilation failed"
    success "TypeScript check passed"

    # Run unit tests
    log "Running unit tests..."
    pnpm test:coverage || error "Unit tests failed"
    ...
    pnpm test:integration || error "Integration tests failed"
...
build_application() {
    ...
    # Build Next.js app (build:docker skips DB operations — migrations run separately)
    log "Building Next.js application..."
    NODE_ENV=production pnpm build:docker || error "Build failed"
...
migrate_database() {
    ...
    pnpm exec prisma db execute --file=<(echo "SELECT 1;") || error "Database connection failed"
    success "Database connection OK"

    # Run migrations (safe, idempotent — never use db push in production)
    log "Applying Prisma migrations..."
    pnpm exec prisma migrate deploy || error "Database migration failed"
...
deploy_docker() {
    ...
    # Verify no secrets in build layers
    log "Checking for secret leaks in Docker layers..."
    if docker history "$FULL_IMAGE" --no-trunc | grep -iE "(SECRET|PASSWORD|API_KEY|TOKEN)" | grep -v "DATABASE_URL=postgresql://build"; then
        error "Potential secret leak detected in Docker layers"
    fi
    success "No secrets found in Docker layers"

    # Push to registry (both staging and production)
...
deploy_cloud_run() {
    ...
    REGION="us-east1"
    SERVICE_ACCOUNT="flipper-run@axovia-flipper.iam.gserviceaccount.com"
    ...
    gcloud run deploy "$SERVICE_NAME" \
        ...
        --set-secrets="DATABASE_URL=DATABASE_URL:latest,AUTH_SECRET=AUTH_SECRET:latest,NEXTAUTH_URL=NEXTAUTH_URL:latest,ENCRYPTION_SECRET=ENCRYPTION_SECRET:latest" \
    ...
    for i in $(seq 1 $retries); do
        ...
        if curl -sf "${SERVICE_URL}/api/health" > /dev/null 2>&1; then
            success "Liveness check passed: /api/health"

            # Readiness check (may fail if DB not yet connected)
            if curl -sf "${SERVICE_URL}/api/health/ready" > /dev/null 2>&1; then
                success "Readiness check passed: /api/health/ready"
            else
                warning "Readiness check returned non-200 (DB may not be connected yet)"
            fi
            return 0
        fi
