  ...
  try {
    ...
    for (let i = 0; i < listings.length; i++) {
      ...
      try {
        ...
        const marketData = await fetchMarketPrice(cleanQuery, identification.category);
    ...
    console.log(
      `  TRUE POSITIVES (eBay confirms profitable): ${truePositives.length} (${Math.round((truePositives.length / withEbayData.length) * 100)}%)`
    );
    console.log(
      `  FALSE POSITIVES (eBay says NOT profitable): ${falsePositives.length} (${Math.round((falsePositives.length / withEbayData.length) * 100)}%)`
    );
    console.log(`No eBay data found: ${noData.length}`);

    if (truePositives.length > 0) {
      const avgProfit = Math.round(
        truePositives.reduce((s, r) => s + (r.trueProfit || 0), 0) / truePositives.length
      );
    ...
    if (falsePositives.length > 0) {
      ...
      for (const fp of falsePositives) {
        console.log(
          `  [${fp.algorithmicScore}] $${fp.askingPrice} → Est $${fp.algorithmicEstimate} vs eBay $${fp.ebayMedian} | True profit: $${fp.trueProfit} | ${fp.title.slice(0, 60)}`
        );
      }
    }
    ...
    if (withEbayData.length > 0) {
      const errors = withEbayData.map((r) =>
        Math.abs((r.algorithmicEstimate - (r.ebayMedian || 0)) / (r.ebayMedian || 1))
      );
