...
<activation critical="MANDATORY">
      ...
      <step n="4">Remind user that all outputs save to _bmad-output/planning-artifacts/</step>
  <step n="5">When creating epics/stories, ensure each maps to specific FR-* requirement numbers</step>
  <step n="6">After PRD is complete, ask user if they want user-flows.md generated</step>
  <step n="7">After epics are defined, create Feature cards in Trello with story checklists</step>
  ...
  <memories>
    <memory>All planning artifacts save to _bmad-output/planning-artifacts/</memory>
    <memory>PRD saves to _bmad-output/planning-artifacts/PRD.md (uppercase)</memory>
    <memory>All functional requirements numbered as FR-&lt;number&gt; (e.g., FR-001)</memory>
    <memory>All non-functional requirements numbered as NFR-&lt;number&gt;</memory>
    <memory>User stories must include Gherkin-style acceptance criteria (Given/When/Then)</memory>
    <memory>CUSTOM: Also produce user-flows.md at _bmad-output/planning-artifacts/user-flows/user-flows.md</memory>
    <memory>User flows document must define all happy paths, alternative paths, and error paths</memory>
    <memory>Each user story acceptance criteria must reference the FR-* requirement IDs it validates</memory>
    <memory>When epics are defined, create a Feature card in Trello Features list: F-&lt;NNN&gt; - &lt;Feature Name&gt;</memory>
    <memory>Feature name is a public-facing name for the epic</memory>
  </memories>
  <menu>
