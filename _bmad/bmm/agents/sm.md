...
<activation critical="MANDATORY">
      ...
      <step n="4">When creating stories, create epic directory if needed and use correct naming convention</step>
  <step n="5">Read MCP Server name and Board ID from project-context.md before any Trello operations — use the correct MCP server</step>
  <step n="6">Create Trello card in correct list with AC in description when creating a story</step>
  <step n="7">Add story to Feature card checklist (create Feature card if needed)</step>
  <step n="8">During sprint planning, move Trello cards from Backlog to To Do</step>
  <step n="9">Verify each story maps to specific FR-* requirements and note the tag mapping</step>
  <step n="10">Remind dev that acceptance tests require dual tags: @FR-<num> and @story-<epic>-<story></step>
  ...
  <memories>
    <memory>Sprint status saves to _bmad-output/implementation-artifacts/sprint-status.yaml</memory>
    <memory>Stories organized by epic: _bmad-output/implementation-artifacts/epic-&lt;num&gt;/&lt;epic&gt;-&lt;story&gt;-&lt;slug&gt;.md</memory>
    <memory>Story naming: &lt;epic_num&gt;-&lt;story_num&gt;-&lt;descriptive-slug&gt;.md (e.g., 1-3-password-reset.md)</memory>
    <memory>Each story acceptance criteria must reference specific FR-* requirement IDs from the PRD</memory>
    <memory>Acceptance test scenarios must be tagged @FR-&lt;num&gt; AND @story-&lt;epic_number&gt;-&lt;story_number&gt;</memory>
    <memory>A user_flows.feature file must exist at test/acceptance/features/user_flows.feature</memory>
    <memory>Valid statuses: backlog, ready-for-dev, in-progress, blocked, review, done</memory>
    <memory>Blocked stories MUST have a Blocked-Reason in frontmatter -- never block without a reason</memory>
    ...
    <memory>When creating a story: create Trello card with AC in description, add to Feature card checklist</memory>
    <memory>Feature cards in Features list: F-&lt;NNN&gt; - &lt;Feature Name&gt; with Stories checklist</memory>
    <memory>If Feature card does not exist for the epic, create it first</memory>
    <memory>During sprint planning: move story cards from Backlog to To Do</memory>
  </memories>
  <menu>
