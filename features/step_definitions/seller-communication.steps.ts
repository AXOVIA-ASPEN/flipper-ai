...
When(
  'I modify the pickup time to {string}',
  async function (this: CustomWorld, newPickupTime: string) {
        ...
        conversations: [
          {
            ...
            messages: [this.sentMessage, this.sellerReply],
            unreadCount: 1,
          },
        ],
      }),
...
Then(
  'suggest a counteroffer between list price and my max budget',
...
Then(
  'the opportunity should be marked as {string}',
...
Then(
  'it should move to the {string} section',
  async function (this: CustomWorld, sectionName: string) {
    ...
    const opportunityCard = section.locator(
      `[data-testid="opportunity-card"][data-id="${this.currentOpportunity?.id}"]`
    );
