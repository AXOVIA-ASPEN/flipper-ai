...
describe('Facebook Scraper Functions', () => {
  ...
  describe('scrapeFacebookMarketplace', () => {
    ...
    it('navigates to correct URL with keywords', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('query=iphone+pro'));
    });

    it('navigates to correct URL with location', async () => {
      await scrapeFacebookMarketplace({ location: 'sarasota' });

      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('/marketplace/sarasota'));
    });

    it('navigates to correct URL with category', async () => {
      await scrapeFacebookMarketplace({ category: 'electronics' });

      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('/electronics'));
    });
    ...
    it('applies sort order', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('sortBy=PRICE_ASCEND'));
    });
    ...
    it('applies price_high sort', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('sortBy=PRICE_DESCEND'));
    });

    it('applies best_match sort', async () => {
      await scrapeFacebookMarketplace({ sortBy: 'best_match' });

      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('sortBy=BEST_MATCH'));
    });
    ...
    it('fetches details when includeDetails is true', async () => {
      ...
      mockExtract
        .mockResolvedValueOnce({
          listings: [{ title: 'Item 1', price: '$50', location: 'NYC' }],
        })
        .mockResolvedValue({
          title: 'Detailed Item',
          price: '$50',
    ...
    it('preserves preview location when detail location is missing', async () => {
      mockExtract
        .mockResolvedValueOnce({
          listings: [{ title: 'Item 1', price: '$50', location: 'Preview Location' }],
        })
        .mockResolvedValueOnce({
          title: 'Item 1',
          price: '$50',
    ...
    it('builds URL without query params when none specified', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith('https://www.facebook.com/marketplace/search');
    });

    it('uses custom category in URL when not in FACEBOOK_CATEGORIES', async () => {
      await scrapeFacebookMarketplace({ category: 'custom-cat' });

      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('/custom-cat'));
    });
  });
  ...
  describe('scrapeAndConvert', () => {
    ...
    it('passes config through to scraper', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('query=laptop'));
    });
  ...
  describe('URL building (indirect tests via navigation)', () => {
    it('combines location and category', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('/marketplace/miami/vehicles'));
    });
    ...
    it('uses /search path when no category specified', async () => {
      ...
      expect(mockGoto).toHaveBeenCalledWith(expect.stringContaining('/marketplace/search'));
    });
  ...
  describe('generateExternalId (indirect)', () => {
    ...
    it('extracts ID from URL-like title with /item/ pattern', () => {
      const result = convertToRawListing({ title: 'Check /item/12345678 out', price: '$10' }, 0);
