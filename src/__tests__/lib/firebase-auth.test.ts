...
describe('lib/firebase/auth', () => {
  ...
  describe('signInWithEmail()', () => {
    ...
    it('should propagate Firebase auth errors', async () => {
      ...
      await expect(authModule.signInWithEmail('user@test.com', 'wrong')).rejects.toThrow(
        'auth/wrong-password'
      );
    });
    ...
    it('should throw when session exchange fails with error detail', async () => {
      ...
      await expect(authModule.signInWithEmail('user@test.com', 'password123')).rejects.toThrow(
  ...
  describe('signUpWithEmail()', () => {
    ...
    it('should propagate account creation errors', async () => {
      ...
      await expect(authModule.signUpWithEmail('existing@test.com', 'pass')).rejects.toThrow(
    ...
    it('should throw when session exchange fails after sign-up', async () => {
      ...
      await expect(authModule.signUpWithEmail('new@test.com', 'pass')).rejects.toThrow(
        'Server error'
      );
    });
  });
  ...
  describe('signInWithGoogle()', () => {
    ...
    it('should propagate popup errors (e.g., user cancelled)', async () => {
      ...
      await expect(authModule.signInWithGoogle()).rejects.toThrow('auth/popup-closed-by-user');
    });
  });
  ...
  describe('signInWithFacebook()', () => {
    ...
    it('should skip storing FB token when oauthCredential is null', async () => {
      ...
      expect(global.fetch).not.toHaveBeenCalledWith('/api/auth/facebook/token', expect.any(Object));
  ...
  describe('resetPassword()', () => {
    ...
    it('should call sendPasswordResetEmail with the correct auth and email', async () => {
      ...
      expect(mockSendPasswordResetEmail).toHaveBeenCalledWith(mockAuthInstance, 'forgot@test.com');
    });
  ...
  describe('exchangeTokenForSession() error handling', () => {
    ...
    it('should use error.detail from response body when available', async () => {
      ...
      await expect(authModule.signInWithEmail('user@test.com', 'pass')).rejects.toThrow(
