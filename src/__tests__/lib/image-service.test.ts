...
describe('Image Download and Caching', () => {
  ...
  describe('downloadAndCacheImage', () => {
    ...
    it("creates cache directory if it doesn't exist", async () => {
      ...
      expect(mkdir).toHaveBeenCalledWith(expect.stringContaining('public/images/listings'), {
        recursive: true,
      });
    });
  });
  ...
  describe('downloadAndCacheImages', () => {
    ...
    it('handles partial failures with skipOnFailure=false', async () => {
      const urls = ['https://example.com/good.jpg', 'https://example.com/bad.jpg'];
  ...
  describe('isImageCached', () => {
    ...
    it('checks multiple extensions', async () => {
      ...
      expect(mockAccess).toHaveBeenCalledWith(expect.stringContaining('test-hash.jpg'));
      expect(mockAccess).toHaveBeenCalledWith(expect.stringContaining('test-hash.png'));
      expect(mockAccess).toHaveBeenCalledWith(expect.stringContaining('test-hash.webp'));
      expect(mockAccess).toHaveBeenCalledWith(expect.stringContaining('test-hash.gif'));
    });
  });
