...
import { DollarSign, TrendingUp, Star, ExternalLink, Send } from 'lucide-react';
...
const COLUMNS = [
  {
    id: 'IDENTIFIED',
    label: 'New',
    badgeClass: 'fp-badge fp-badge-blue',
    headerColor: 'rgba(96,165,250,0.15)',
    borderColor: 'rgba(96,165,250,0.25)',
  },
  {
    id: 'CONTACTED',
    label: 'Contacted',
    badgeClass: 'fp-badge fp-badge-yellow',
    headerColor: 'rgba(251,191,36,0.12)',
    borderColor: 'rgba(251,191,36,0.25)',
  },
  {
    id: 'PURCHASED',
    label: 'Purchased',
    badgeClass: 'fp-badge fp-badge-purple',
  ...
  },
  {
    id: 'LISTED',
    label: 'Listed',
    badgeClass: 'fp-badge fp-badge-orange',
    headerColor: 'rgba(251,146,60,0.12)',
    borderColor: 'rgba(251,146,60,0.25)',
  },
  {
    id: 'SOLD',
    label: 'Sold',
    badgeClass: 'fp-badge fp-badge-green',
  ...
  },
  {
    id: 'PASSED',
    label: 'Passed',
    badgeClass: 'fp-badge fp-badge-gray',
    headerColor: 'rgba(148,163,184,0.08)',
...
export default function KanbanBoard({
  ...
  const groupedOpportunities = React.useMemo(() => {
    ...
    for (const opp of opportunities) {
      // Map legacy statuses
      const status =
        opp.status === 'NEW' || opp.status === 'OPPORTUNITY' ? 'IDENTIFIED' : opp.status;
      if (grouped[status]) {
    ...
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="flex gap-4 overflow-x-auto pb-4" data-testid="kanban-board">
        {COLUMNS.map((col) => {
          ...
          const showCrossPost = !!onCrossPost && CROSS_POST_ALLOWED_STATUSES.has(col.id);
          return (
            <div key={col.id} className="flex-shrink-0 w-72" style={{ borderRadius: 12 }}>
              {/* Column Header */}
              <div
                style={{
                  padding: '12px 16px',
                          ...
                          {(provided, snapshot) => (
                            ...
                            >
                              ...
                              <div
                                ...
                                style={{ padding: image ? '0 12px 8px' : '12px 12px 0' }}
                              >
                              ...
                              >
                                ...
                                {opp.listing.valueScore != null && (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    ...
                                    <span
                                      style={{ fontWeight: 600, fontSize: 13, color: '#8b5cf6' }}
                                    >
                                      {opp.listing.valueScore.toFixed(0)}
                                    </span>
                                    {opp.listing.demandLevel &&
                                      DEMAND_BADGES[opp.listing.demandLevel] && (
                                        <span
                                          className={
                                            DEMAND_BADGES[opp.listing.demandLevel].className
                                          }
                                          style={{ fontSize: 9 }}
