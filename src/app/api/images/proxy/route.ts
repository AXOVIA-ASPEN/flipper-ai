...
export async function GET(request: NextRequest) {
  try {
    ...
    if (cachedPath) {
      ...
      return NextResponse.redirect(new URL(cachedPath, request.nextUrl.origin), { status: 302 });
    }

    // Fetch the image and optionally cache it
    const downloadResult =
      searchParams.get('cache') !== 'false' ? await downloadAndCacheImage(imageUrl) : null;
