...
export async function GET() {
  return NextResponse.json({
    ...
    notes: "Requires user-specific Facebook OAuth token. Token must be stored in FacebookToken table.",
    authRequired: true,
  });
}
    ...
    try {
      ...
      await prisma.scraperJob.update({
        ...
        data: {
          status: "FAILED",
          errorMessage:
            error instanceof Error ? error.message : "Unknown Facebook API error",
          completedAt: new Date(),
        },
      });
      throw error;
