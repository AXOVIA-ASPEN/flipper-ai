...
test.describe("Feature: Complete Flip Journey", () => {
  ...
  test("Scenario: Given a user with opportunities, When they update status through the flip lifecycle, Then each stage is tracked", async ({
        ...
        await route.fulfill({
          json: {
            opportunities: [
              {
                ...
                purchaseDate:
                  currentStatus === "PURCHASED" ? new Date().toISOString() : null,
                resalePrice: currentStatus === "SOLD" ? 260 : null,
                resaleDate:
                  currentStatus === "SOLD" ? new Date().toISOString() : null,
            ...
            stats: {
              ...
              totalInvested:
                ["PURCHASED", "LISTED", "SOLD"].includes(currentStatus) ? 110 : 0,
    ...
    expect(statusHistory).toEqual(
      expect.arrayContaining(["CONTACTED", "PURCHASED", "SOLD"])
    );
  });
